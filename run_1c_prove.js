...
(async () => {
  ...
  const server = http.createServer((req, res) => {
    const requested = req.url === '/' ? '/tests/1c.html' : req.url;
  ...
  await page.goto(`http://127.0.0.1:${port}/tests/1c.html`, {waitUntil:'networkidle0',timeout:30000});
  ...
  const outDir=path.join(__dirname,'prove-artifacts-1c'); fs.mkdirSync(outDir,{recursive:true});
